...
from Scripts.utils import force_exit, log

print("""This is used to convert v1 saves to v2, as it uses a completely different format.
Instructions:
1. Place this file in the same folder as your 'save.json'.
    ...
    try:
        ...
        if choice == "READY":
            ...
            if not exists("save.json"):
                force_exit("Save file could not be found.")
                
            if exists("save.toml"):
                force_exit("v2 save file already exists.")
            ...
            if not old_save.get("checksum", False):
                force_exit("JSON file is invalid or missing checksum.")
            ...
            if old_save["checksum"][1] == "/":
                ...
                force_exit(f"Invalid symbol further into execution, got '{old_save['checksum'][1]}'.")
